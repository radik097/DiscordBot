...
- 🕘 Отдельная история реально проигранных закэшированных песен с количеством
  запусков и повторным включением без создания плейлиста
...
- 📝 Локальная транскрипция Discord Voice через Whisper small: подписи
  говорящих, минутные чанки, TXT/SRT, веб-панель и подавление музыки бота
...
/download url:<ссылка> format:<video|audio> quality:<480|720|1080|1440|2160|max>  скачать видео или аудио
/transcribe start language:<auto|ru|en>  начать запись текущего голосового канала
/transcribe status|stop                 состояние или остановка записи
/transcribe export format:<txt|srt>     экспорт текста или субтитров
/transcribe delete session:<id>         удалить текст и сохранённое аудио
...
умолчанию `Ботоводство`). Можно запросить видео со звуком в H.264/MP4 с верхней
границей 480p, 720p, 1080p, 1440p, 2160p либо максимальным доступным качеством,
или сохранить только аудио. Бот принимает только публичные HTTP(S)-ссылки без
учётных данных, передаёт их собственному Cobalt-контейнеру и не обходит DRM,
paywall или доступ к приватным материалам. В зависимости от актуальной поддержки
Cobalt подходят публичные видео YouTube, TikTok, Instagram, X, Reddit, Vimeo,
VK и других сервисов.

Маленький файл из Discord-команды отправляется приватным вложением с учётом
фактического `attachment_size_limit`; крупный получает случайную временную
ссылку панели. В разделе «Загрузки» ту же операцию можно запустить из браузера:
задача сразу попадает в фоновую очередь, а ссылка появляется в истории после
завершения. Полный URL с query-параметрами в истории не хранится. По умолчанию
действуют лимит 500 МБ, две одновременные загрузки, очередь 20 и cooldown 30
секунд. Cobalt не публикует порт хоста и доступен только боту в Docker-сети.

`/transcribe` доступна только администратору или роли `Ботоводство`. Старт и
остановка публично видны в текстовом канале; индивидуальное подтверждение
участников не запрашивается, поэтому оператор должен заранее соблюдать правила
сервера и применимое законодательство. Бот принимает каждого говорящего
отдельным Discord Voice-потоком, буферизует по одной минуте с перекрытием 1 секунда
и обрабатывает локальным `Whisper small`. Режим `auto` ограничивает определение
русским и английским, также язык можно зафиксировать явно.

Во время музыки бот передаёт worker-у отдельный reference-поток и применяет
SpeexDSP echo cancellation. При слабой корреляции или ошибке AEC исходная речь
сохраняется, чтобы агрессивная фильтрация не вырезала говорящего; поэтому полное
удаление музыки при акустическом эхо не гарантируется. Обработанный текст хранится
в SQLite, исходные дорожки архивируются во FLAC и автоматически удаляются через
7 дней. TXT/SRT остаются доступны после удаления аудио. Веб-раздел
«Транскрипция» умеет запускать и останавливать запись, показывать готовые минутные
части и выгружать результат.
...
Кнопка «🎧 Включить мониторинг» переводит форму в автономный режим: вставьте
трек или ссылку и нажмите «🎧 Слушать», чтобы подготовить и воспроизвести файл
прямо в браузере без голосового канала и Discord Voice. Если в Discord уже
играет закэшированный трек, монитор также может подключиться к нему с текущей
позиции. При смене трека или уровня отдельный защищённый MP3-поток автоматически
пересоздаётся с выбранной серверной громкостью.
...
Раздел «История песен» ведётся независимо от плейлистов: после успешного запуска
аудиопотока бот сохраняет метаданные и имя фактического cache-файла в SQLite.
Одна песня занимает одну позицию со счётчиком запусков. Если очистка кэша удалит
файл, запись автоматически исчезнет; оставшуюся песню можно снова включить из
панели без повторного скачивания.
