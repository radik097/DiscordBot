...
Текущая версия: **1.10.3**. Подробное описание изменений — в
...
### Docker
```bash
docker compose up -d --build               # собрать/запустить 1.10.3
...
```
/play query:<трек, запрос или плейлист> либо file:<аудио/видео>
/skip    /pause    /resume    /stop
/queue                        /nowplaying         /volume <0-200>
/phone                        одноразовая ссылка для входа с телефона (только роль Ботовод)
/get-url email:<почта>        одноразовая ссылка на сутки (только роль Ботовод)
...
`Ботовод` (регистр роли не важен). Ссылка отправляется ephemeral-ответом, живёт
пять минут и удаляется после первого успешного подтверждения на телефоне.
Администратор без роли `Ботовод` получить ссылку не может.

`/get-url` также требует роль `Ботовод` и отвечает только автору команды.
...
`/play` принимает либо `query` с обычной ссылкой/поиском/YouTube-плейлистом,
либо `file` с вложением Discord. Файл потоково сохраняется в аудиокэш,
...
через `MUSIC_FILE_MAX_BYTES`. Параметр `list=...` или ID плейлиста добавляет до
500 доступных роликов в очередь по порядку.
