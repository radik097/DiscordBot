...
describe("transcription service", () => {
  ...
  test("captures one Discord speaker, processes a chunk and keeps the voice lease independent", async () => {
    ...
    const service = new TranscriptionService({ root, worker, queueProvider: () => queue, chunkMs: 60_000 });
    let session;
    try {
      session = await service.start({ guild, voiceChannel, announceChannel, language: "auto", startedById: "operator", startedByTag: "Operator" });
