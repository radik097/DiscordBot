...
function interaction({ roles = [], admin = false, attachmentSizeLimit = 10_000 } = {}) {
  const calls = [];
  return {
    ...
    options: { getString: () => "https://youtube.com/watch?v=abc" },
...
describe("/download access", () => {
  ...
  test("uploads a small result and removes its temporary file", async () => {
    const removed = [];
    const service = {
      run: async () => ({ id: "job", path: "C:/tmp/clip.mp4", filename: "clip.mp4", size: 9000, itemCount: 1 }),
